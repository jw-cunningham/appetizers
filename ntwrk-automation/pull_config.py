...
#global libraries
from flytools import securetools #custom toolkit
...
LOG_FILE = PRJ_FOLDER + 'pull_' + console_arg + '_' + TIME_STRING + '.log'
CMD_FILE = PRJ_FOLDER + 'pull_cmds.txt'
INV_FILE = PRJ_FOLDER + 'target.txt'
log_list = []

### Functions ###
def save_to_grab(grab_file, grab_list):
    # expects a grab file a device string and a grab_list of dictionaries with 'ios_command' and 'output' keys
        with open(grab_file,'w') as grab_f:
                for dict in grab_list:
                        grab_f.write('\n#########################  {ios_command}  #########################\n'.format(**dict))
                        grab_f.write(dict['output']+'\n')

def pull_from_device(q,usr,pwd):
    # router login info
        ...
        if connection_successful:
            for command in CMD_LIST:
                try:
                    ...
                    error_fecting_data = True
                # append command results to list
                command_results_list.append({'ios_command':command.strip(),'output':output})
            run_output = connection.send_command("show running-config")
            connection.disconnect()

            #save to grab file
            save_to_grab(PRJ_FOLDER + device + '.grab', command_results_list)

            #save run_output to a running config file
            run_file = open(PRJ_FOLDER + device + '.run', 'w')
            run_file.write(run_output)
            run_file.close()

            #save to log file
            if error_fetching_data:
                log_list.append(f'\nIncomplete | {device} | Not all data was pulled succesfully')

            else:
                log_list.append(f'\nOK | {device} | pulling data was successful')
        q.task_done()
...
for i in range(number_of_threads):
        t = Thread(target = pull_from_device, args = (q,usr,pwd))
        t.start()
q.join()
